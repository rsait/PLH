...
open(FITX,$ARGV[0]) or die("Errorea! Ezin fitxategia zabaldu\n");
@puntuak = <FITX>;
close(FITX);
...
foreach $lerro(@puntuak) {
	...
	@hitzak = split(/[\n\s-]/, $lerro);
	$oina = pop(@hitzak);
        $errima = errima_patroia($oina);
	if (not length $errima) {
		push(@errimaksentences, $lerro);
	}
}
foreach ( @errimaksentences ) {
	    print $_, "\n";
	}
